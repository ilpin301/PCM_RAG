...
**Status:** **S1 done** (2026-08-25). Kit repo at `F:\____IL_AI\RAG\ragkit`, commit `4e47bfa`.
Scripts were **copied**, not moved — both bases still run on their original launchers, untouched.
Deviations from the plan text: (a) the python takes its base from the `RAGBASE_ROOT` env var only,
set by `ingest.ps1 -Root`; no per-script `--root` flag was added, and a small `ragbase.py` holds the
root/storage/.env resolution the scripts share. (b) this machine had **no** `data-gym-cache` left in
`%TEMP%`, so the tiktoken seed was generated by downloading `o200k_base`/`cl100k_base`/`gpt2` into
`ragkit\seed\` rather than copied. (c) `.gitattributes` sets `* -text`; CRLF conversion would
corrupt the seed. (d) of `repairs\*.ps1` only `cleanup_processed_slices.ps1` was adopted (and given
`-Root`); `cleanup_leftovers.ps1`, `delete_dup_stub.ps1` and `refresh_drive_snapshot.ps1` are one-off
PCM incident scripts and stayed in PCM_RAG, as `resync_chunks_count.py` already did by plan.
(e) `bootstrap.ps1` was run early (it installs skills, of which there are none until S2) so that
`machine.ps1` existed for guard testing. `RAGKIT_HOME` is **not yet set** — S2 needs it.
Next: S2.
