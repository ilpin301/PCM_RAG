...
deletes on 2026-08-20, each exactly the `vdb_entities` minus graph-node gap. Do not rank this as
corruption or crash wreckage when a document was recently deleted; rank it `warning` and say plainly
that a delete happened.

**Do not write a new repair script for it.** `F:\____IL_AI\PCM_RAG\lightrag\repairs\repair_vdb.py`
already exists, already defaults to a dry run, and already writes `.bak` files for all three stores
before touching anything. Point the user at it instead:

```sh
python repairs/repair_vdb.py            # dry run
python repairs/repair_vdb.py --apply
```
